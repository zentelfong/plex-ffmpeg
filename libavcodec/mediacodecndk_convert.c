...
static size_t _qcom_tile_pos(size_t x, size_t y, size_t w, size_t h)
{
  size_t flim = x + (y & ~1) * w;
    ...
    for(size_t y = 0; y < tile_h_luma; y++) {
        ...
        for(size_t x = 0; x < tile_w; x++) {
            ...
            const uint8_t *src_luma  = data
                + _qcom_tile_pos(x, y,tile_w_align, tile_h_luma) * QCOM_TILE_SIZE;

            /* chroma source pointer for this tile */
            const uint8_t *src_chroma = data + luma_size
                + _qcom_tile_pos(x, y/2, tile_w_align, tile_h_chroma) * QCOM_TILE_SIZE;
            if (y & 1)
